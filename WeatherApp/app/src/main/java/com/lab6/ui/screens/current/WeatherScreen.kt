...
fun WeatherScreen(
    ...
    LazyColumn(
        ...
        item {
            // using of CircularProgressIndicator in order to asynchronous getting of data
            if (isLoading) {
                ...
                weatherResponseState?.let { weatherResponse ->
                    if (weatherResponse.main != null && weatherResponse.weather.isNotEmpty()) {
                       ...
                       // row with city name which can be clicked and the
                        // openCityInputDialog method is being called
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                        ...
                        WeatherMainCustomView(
                            // getting the the weather data object from weatherMain and
                    ...
                    item {
                        ...
                        LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            items(forecasts) { dailyForecast ->
                                ...
                                // This component is used to display weather forecast data for a specific hour.
                                WeatherForecastCard(
                                    hour = hour,
                                    temperature = tempCelsius,
                                    iconCode = weatherDesc?.icon ?: "01d"
                                    // if weatherDesc is null, then display clear sky icon
                                )
                            }
                        }
                    }
...
// open button for changing of city
    if (isDialogOpen) {
        CityInputDialog(
